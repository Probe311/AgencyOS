...
export class AmazonAdsService extends BaseIntegrationService {
  ...
  async getAccountInfo(accessToken: string): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        return {
          id: 'amazon_ads_account',
          name: 'Compte Amazon Ads',
        };
      }
      ...
      return {
        id: profile?.profileId?.toString() || 'amazon_ads_account',
        name: profile?.accountInfo?.marketplaceStringId || 'Compte Amazon Ads',
      };
    } catch {
      return {
        id: 'amazon_ads_account',
